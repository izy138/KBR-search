...
const VIEWBOX_PAD_X = 48;
const VIEWBOX_PAD_Y = 52;
const STATIC_VIEWBOX = `${-VIEWBOX_PAD_X} ${-VIEWBOX_PAD_Y} ${SVG_W + VIEWBOX_PAD_X * 2} ${SVG_H + VIEWBOX_PAD_Y * 2}`;
const GLOBAL_BREATHE = 10;
const MAX_PUSH = 34;
...
const GROUP_MOVE_TRANSITION = `transform ${LAYOUT_MS}ms ${LAYOUT_EASE}`;
const CAT_RX = 12;
const SUB_RX = 8;
...
function catNodeBox(weight: number, maxWeight: number, lines: string[], fontSize: number): NodeBox {
  const t = maxWeight > 0 ? weight / maxWeight : 0;
  const weightBox: NodeBox = { halfW: 40 + t * 20, halfH: 24 + t * 12 };
  const textBox = boxForInsideText(lines, fontSize, 40, 92, 22, 56, 12, 10);
  return {
    halfW: Math.max(weightBox.halfW, textBox.halfW),
    halfH: Math.max(weightBox.halfH, textBox.halfH),
  };
}
...
function connectorPath(
  ...
  const [x1, y1] = rectEdgePoint(catX, catY, catBox.halfW, catBox.halfH, subX, subY);
  const [x2, y2] = rectEdgePoint(subX, subY, subBox.halfW, subBox.halfH, catX, catY);
  const mx = (x1 + x2) / 2;
  const my = (y1 + y2) / 2;
  const dx = subX - catX;
  const dy = subY - catY;
...
function subNodeBox(
  ...
  const subLines = splitLabel(sub.label, 12);
  const subT = (sub.weight ?? 0) / subMaxW;
  const textBox = boxForInsideText(subLines, subFontSize, 18, 44, 12, 28, 8, 6);
  const weightBox: NodeBox = { halfW: 16 + subT * 6, halfH: 11 + subT * 4 };
  const base: NodeBox = {
    halfW: Math.max(textBox.halfW, weightBox.halfW),
    halfH: Math.max(textBox.halfH, weightBox.halfH),
  };
  return selected
    ? { halfW: base.halfW + 2, halfH: base.halfH + 2 }
    : base;
}
...
function computeCategoryNudge(
  catNodes: CatNodeLayout[],
  activeId: string | null,
): Map<string, { dx: number; dy: number }> {
  const offsets = new Map<string, { dx: number; dy: number }>();
  const activeNode = catNodes.find((node) => node.cat.id === activeId);

  for (const node of catNodes) {
    if (!activeNode || node.cat.id === activeId) {
      offsets.set(node.cat.id, { dx: 0, dy: 0 });
      continue;
    }

    const subs = activeNode.cat.children ?? [];
    if (subs.length === 0) {
      offsets.set(node.cat.id, { dx: 0, dy: 0 });
      continue;
    }

    const deltaAngle = angularDistanceDeg(node.angleDeg, activeNode.angleDeg);
    const fanHalf = SUB_FAN_DEG / 2 + 30;
    const nodeSpan = Math.max(node.box.halfW, node.box.halfH);
    const nodeHalfWidth = ((Math.atan2(nodeSpan, R_CAT) * 180) / Math.PI) + 10;
    const overlap = fanHalf + nodeHalfWidth - deltaAngle;
    let pushDist = GLOBAL_BREATHE;
    if (overlap > 0) {
      const strength = Math.min(1, overlap / (fanHalf * 0.9));
      pushDist = GLOBAL_BREATHE + strength * (MAX_PUSH - GLOBAL_BREATHE);
    }

    const [tx, ty] = radialPos(CX, CY, R_CAT + pushDist, node.angleDeg);
    offsets.set(node.cat.id, { dx: tx - node.x, dy: ty - node.y });
  }
  ...
  return (
    <rect
      ...
      style={{
        ...
        transition: `x ${SUB_OPEN_MS * 0.45}ms ${SUB_OPEN_EASE}, y ${SUB_OPEN_MS * 0.45}ms ${SUB_OPEN_EASE}, width ${SUB_OPEN_MS * 0.45}ms ${SUB_OPEN_EASE}, height ${SUB_OPEN_MS * 0.45}ms ${SUB_OPEN_EASE}, fill ${SUB_OPEN_MS * 0.45}ms ${LAYOUT_EASE}, stroke ${SUB_OPEN_MS * 0.45}ms ${LAYOUT_EASE}`,
      }}
      onClick={onClick}
...
export default function ProjectTermsThemeCloud({ payload, onSearch }: Props): ReactElement {
  ...
  const catNodes = rawTree.map((cat, i) => {
    ...
    const lines = splitLabel(cat.label, 14);
    const box = catNodeBox(cat.weight ?? 0, maxWeight, lines, catFontSize);
    return { cat, x, y, angleDeg, hue, box, lines };
  });
  ...
  const categoryNudge = computeCategoryNudge(catNodes, activeId);
        ...
        <div className="flex w-full min-h-[28rem] lg:min-h-[32.5rem] flex-col lg:flex-row items-stretch overflow-hidden">
          ...
          >
            <div className="w-full self-stretch min-h-[20rem] max-h-[32.5rem] h-full p-1 box-border">
              <svg
                viewBox={STATIC_VIEWBOX}
                preserveAspectRatio="xMidYMid meet"
                className="w-full h-full block max-w-full max-h-full"
                aria-label="Radial theme cloud"
              >
              {catNodes.map(({ cat, x, y, angleDeg, hue, box, lines }) => {
                const isActive = activeId === cat.id;
                const displayBox = isActive ? expandBox(box, 4) : box;
                ...
                const nudge = categoryNudge.get(cat.id) ?? { dx: 0, dy: 0 };

                return (
                  <g
                    key={cat.id}
                    transform={`translate(${nudge.dx} ${nudge.dy})`}
                    style={{ transition: GROUP_MOVE_TRANSITION }}
                  >
                    <ThemeRect
                      cx={x}
                      cy={y}
                    ...
                    {subPositions.map(([sx, sy], si) => {
                      const sub = subs[si];
                      const subBox = sub
                        ? subNodeBox(sub, subMaxW, subFontSize, selectedSubId === sub.id)
                        : { halfW: 18, halfH: 12 };
                      const pathD = connectorPath(x, y, displayBox, sx, sy, subBox);
                    ...
                    {subs.map((sub, si) => {
                      ...
                      const subBox = subNodeBox(sub, subMaxW, subFontSize, isSubSelected);
